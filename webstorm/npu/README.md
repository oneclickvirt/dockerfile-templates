...
| Item                              | Link                                                                                      |
|-----------------------------------|-------------------------------------------------------------------------------------------|
| WebStorm-2024.1-aarch64.tar.gz | [Download Link](https://download.jetbrains.com/webstorm/WebStorm-2024.1-aarch64.tar.gz)   |
...
# JetBrains WebStorm + NPU + AI插件 镜像构建流程  
...
执行以下命令创建镜像：  
```bash
docker build -t jetbrains-webstorm-plug-npu:v2024.2.1-arm .
```

## 3. 启动容器并安装插件  
### 3.1 运行容器  
```bash
docker run -d -p 0.0.0.0:31000:31000 --name=webstorm jetbrains-webstorm-plug-npu:v2024.2.1-arm
```
...
### 4.1 提交修改后的容器为新镜像  
```bash
docker commit webstorm jetbrains-webstorm-plug-npu:v2024.2.1-arm
```
